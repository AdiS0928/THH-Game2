...
function setup() {
    const imageParam = 'static/Artwork.jpg'
    ...
    if(window.innerWidth < 1000){
        for(var x=0; x<cols; x++) {
            for(var y=0; y<rows; y++) {
                ...
                switch(orientation) {
                    case HORIZONTAL:
                        positionX = side * preview.width/2 * random(1.5, 3.00)
                        positionY = random(-preview.height/2 * 1.15, preview.height/2 * 1.15)
                        break;
                    case VERTICAL:
                        positionX = random(-preview.width/2 * 1.0, preview.width/2 * 1.0)
                        positionY = side * preview.height/2 * random(1.3, 2.5)
                        break;
                }
